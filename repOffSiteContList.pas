...
type
  Trep_OffSiteContList = class(TForm)
...
var
  rep_OffSiteContList: Trep_OffSiteContList;
...
procedure Trep_OffSiteContList.lbl_RangePrint(sender: TObject;
  var Value: String);
begin
  Value := 'From: '+DatetoStr(FromD)+' to '+DatetoStr(ToD);
end;

procedure Trep_OffSiteContList.qry_Get_Br_Dpt_FTAfterScroll(
  DataSet: TDataSet);
begin
//============Total Containers Captured=========================================
  ...
  qry_GetTot.SQL.Add('AND (b.Transaction_Type = ''COS'')');
...
procedure Trep_OffSiteContList.QRBand8BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
