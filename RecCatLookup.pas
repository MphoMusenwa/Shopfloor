...
type
  ...
  private
    ...
    sColumnHead, sRecordCat :String;
    sColumnType            :String;
  public
    { Public declarations }
    TextString : String;
    Editable   :Boolean;
    function Execute(ColumnHeading, ColumnType, RecordCat  : String) : Integer;
  end;

var
  frm_RecCatLookup: Tfrm_RecCatLookup;
...
function Tfrm_RecCatLookup.Execute(ColumnHeading, ColumnType, RecordCat  : String) : Integer;
begin
  sColumnHead   := ColumnHeading;
  sColumnType   := ColumnType;
  Caption := 'Lookup Values for "'+sColumnHead+'"';
  sRecordCat  := RecordCat;
//---
  ...
  try
    qry_Look.Close;
    qry_Look.SQL.Clear;
    qry_Look.SQL.Add('SELECT Item FROM RecCatLookups');
    qry_Look.SQL.Add('WHERE RecordCat = '+QuotedStr(sRecordCat));
...
procedure Tfrm_RecCatLookup.Add1Click(Sender: TObject);
Var
  SStr  :String;
begin
  ...
  If InputQuery('Enter value for Lookup', 'Value:', SStr) Then
    begin
      ...
      try
        qry_Look.Close;
        qry_Look.SQL.Clear;
        qry_Look.SQL.Add('INSERT INTO RecCatLookups (RecordCat,ColumnHeading, Item)');
        qry_Look.SQL.Add('VALUES('+QuotedStr(sRecordCat)+','+QuotedStr(sColumnHead)+',');
...
procedure Tfrm_RecCatLookup.Remove1Click(Sender: TObject);
begin
  If not Editable Then
    exit;
...
//-----
  ...
  try
    qry_Look.Close;
    qry_Look.SQL.Clear;
    qry_Look.SQL.Add('DELETE FROM RecCatLookups');
    qry_Look.SQL.Add('WHERE RecordCat = '+QuotedStr(sRecordCat));
    qry_Look.SQL.Add('AND ColumnHeading = '+QuotedStr(sColumnHead));
...
procedure Tfrm_RecCatLookup.lb_LookupKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If Editable Then
    exit;
...
procedure Tfrm_RecCatLookup.Edit1Click(Sender: TObject);
Var
  SStr  :String;
  FStr  :String;
begin
  ...
  If InputQuery('Enter value for Lookup', 'Value:', SStr) Then
    begin
      ...
      try
        qry_Look.Close;
        qry_Look.SQL.Clear;
        qry_Look.SQL.Add('UPDATE RecCatLookups SET');
        If pos('TEXT', upperCase(sColumnType)) <> 0 Then
