...
procedure Trep_DesRecAmt.QRBand8BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qry_Records.Close;
  ...
  qry_Records.SQL.Add('AND (b.Transaction_Type = ''FDS'')');
...
procedure Trep_DesRecAmt.reportBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  Tot := 0;
end;

procedure Trep_DesRecAmt.lbl_CTotalPrint(sender: TObject;
  var Value: String);
begin
  Value := InttoStr(Tot);
end;

procedure Trep_DesRecAmt.lbl_RangePrint(sender: TObject;
  var Value: String);
begin
