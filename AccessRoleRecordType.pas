...
type
  Tfrm_AccessRoleRecordType = class(TForm)
    ToolBar1: TToolBar;
...
var
  frm_AccessRoleRecordType: Tfrm_AccessRoleRecordType;
...
procedure Tfrm_AccessRoleRecordType.btn_SelectClick(Sender: TObject);
var
  i : Integer;
...
procedure Tfrm_AccessRoleRecordType.DeSelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
...
procedure Tfrm_AccessRoleRecordType.DisplayAccess;
Var
  R  :Integer;
...
//-------------------Build RecordTypes------------------------------------------
  qry_Access.Close;
  qry_Access.SQL.Clear;
  qry_Access.SQL.Add('DECLARE @RoleId int');
  qry_Access.SQL.Add('');
  qry_Access.SQL.Add('SET @RoleId = :RoleId');
  qry_Access.SQL.Add('');
  ...
  qry_Access.SQL.Add('		WHEN id.UseContainers = 1 AND EXISTS(SELECT * FROM IndexUAuthRts x WHERE x.Branch = i.Branch AND x.Department = i.Department AND X.Description = i.Description AND x.RoleId = @RoleId AND x.Containers = 1) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS ContainersAccess,');
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN id.UseFiles = 0 THEN -1');
  qry_Access.SQL.Add('		WHEN id.UseFiles = 1 AND EXISTS(SELECT * FROM IndexUAuthRts x WHERE x.Branch = i.Branch AND x.Department = i.Department AND X.Description = i.Description AND x.RoleId = @RoleId AND x.Files = 1) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS FilesAccess,');
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN id.UseItems = 0 THEN -1');
  qry_Access.SQL.Add('		WHEN id.UseItems = 1 AND EXISTS(SELECT * FROM IndexUAuthRts x WHERE x.Branch = i.Branch AND x.Department = i.Department AND X.Description = i.Description AND x.RoleId = @RoleId AND x.Items = 1) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS ItemsAccess,');
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN id.UseERecords = 0 THEN -1');
  qry_Access.SQL.Add('		WHEN id.UseERecords = 1 AND EXISTS(SELECT * FROM IndexUAuthRts x WHERE x.Branch = i.Branch AND x.Department = i.Department AND X.Description = i.Description AND x.RoleId = @RoleId AND x.eRecords = 1) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS eRecordsAccess,');
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN id.UseCool = 0 THEN -1');
  qry_Access.SQL.Add('		WHEN id.UseCool = 1 AND EXISTS(SELECT * FROM IndexUAuthRts x WHERE x.Branch = i.Branch AND x.Department = i.Department AND X.Description = i.Description AND x.RoleId = @RoleId AND x.COOL = 1) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
...
function Tfrm_AccessRoleRecordType.Execute(RoleId :Integer):Integer;
Var
  TempStr   :String;
begin
  mRoleId := RoleId;
...
procedure Tfrm_AccessRoleRecordType.FormCreate(Sender: TObject);
var 
  bmp: TBitmap; 
begin 
...
procedure Tfrm_AccessRoleRecordType.FormDestroy(Sender: TObject);
begin
  AccessCChk.free;
  AccessFChk.free;
  AccessIChk.free;
...
procedure Tfrm_AccessRoleRecordType.SelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
  For J := 0 to AccessCChk.Count-1 do
...
procedure Tfrm_AccessRoleRecordType.sgrd_AccessClick(Sender: TObject);
var
  pt: TPoint;
  grid: TStringgrid;
  aCol, aRow: Integer;
...
procedure Tfrm_AccessRoleRecordType.sgrd_AccessDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_AccessRoleRecordType.ToggleCheckbox( acol, arow: Integer );
begin
