...
type
  Tfrm_AccessDeliveryType = class(TForm)
    ToolBar1: TToolBar;
    ...
    lbl_Hd: TLabel;
    lbl_DeliveryType: TLabel;
    qry_DeliveryType: TADOQuery;
  ...
  public
    { Public declarations }
    function Execute(DeliveryType :String):Integer;
  end;

var
  frm_AccessDeliveryType: Tfrm_AccessDeliveryType;
...
procedure Tfrm_AccessDeliveryType.btn_SelectClick(Sender: TObject);
var
  i : Integer;
begin
...
//---------------Delete---------------------
  ...
  qry_Access.SQL.Add('   DeliveryType = :DeliveryType');
  qry_Access.Parameters.ParamValues['DeliveryType'] := mDeliveryType;
  qry_Access.ExecSQL;
//---------------Add Users---------------------
        ...
        begin
          If UsersChk.Strings[I-1] = '1' Then
            begin
              qry_Access.Close;
              qry_Access.SQL.Clear;
              qry_Access.SQL.Add('INSERT INTO DTAuthRts (DeliveryType, Code, UserId) VALUES(:DeliveryType, :Code, :UserId)');
              qry_Access.Parameters.ParamByName('DeliveryType').Value := mDeliveryType;
...
procedure Tfrm_AccessDeliveryType.DeSelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
...
procedure Tfrm_AccessDeliveryType.DisplayAccess;
Var
  R  :Integer;
...
//-------------------Build Users-----------------------------------------------
  ...
  qry_Access.SQL.Add('		WHEN EXISTS(SELECT * FROM DTAuthRts x WHERE x.DeliveryType = :DeliveryType AND x.UserId = a.UserId) THEN 1');
...
//-------------------Build Roles-----------------------------------------------
  ...
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN EXISTS(SELECT * FROM DTAuthRts x WHERE x.DeliveryType = :DeliveryType AND x.RoleId = a.RoleId) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS Access');
  qry_Access.SQL.Add('FROM');
...
function Tfrm_AccessDeliveryType.Execute(DeliveryType :String):Integer;
begin
  mDeliveryType := DeliveryType;

  qry_DeliveryType.Close;
  qry_DeliveryType.SQL.Clear;
  qry_DeliveryType.SQL.Add('SELECT Description FROM Delivery WHERE Description = :DeliveryType');
  qry_DeliveryType.Parameters.ParamValues['DeliveryType'] := mDeliveryType;
  qry_DeliveryType.Open;

  lbl_DeliveryType.Caption := qry_DeliveryType.FieldbyName('Description').AsString;
...
procedure Tfrm_AccessDeliveryType.FormCreate(Sender: TObject);
var 
  bmp: TBitmap; 
begin 
...
procedure Tfrm_AccessDeliveryType.FormDestroy(Sender: TObject);
begin
  UsersChk.Free;
  RolesChk.Free;
end;

procedure Tfrm_AccessDeliveryType.MenuItem1Click(Sender: TObject);
Var
   J      :Integer;
begin
...
procedure Tfrm_AccessDeliveryType.MenuItem2Click(Sender: TObject);
Var
   J      :Integer;
begin
  For J := 0 to UsersChk.Count-1 do
...
procedure Tfrm_AccessDeliveryType.SelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
  For J := 0 to RolesChk.Count-1 do
...
procedure Tfrm_AccessDeliveryType.sgrd_RolesClick(Sender: TObject);
var
  pt: TPoint;
  grid: TStringgrid;
  aCol, aRow: Integer;
...
procedure Tfrm_AccessDeliveryType.sgrd_RolesDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_AccessDeliveryType.sgrd_UsersClick(Sender: TObject);
var
  pt: TPoint;
  grid: TStringgrid;
  aCol, aRow: Integer;
...
procedure Tfrm_AccessDeliveryType.ToggleCheckboxUsers( acol, arow: Integer );
begin
...
procedure Tfrm_AccessDeliveryType.ToggleCheckboxRoles( acol, arow: Integer );
begin
...
procedure Tfrm_AccessDeliveryType.sgrd_UsersDrawCell(Sender: TObject; ACol,
  ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  grid: TStringgrid;
begin
