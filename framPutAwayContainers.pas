...
type
  Tfram_PutAwayContainers = class(TFrame)
...
procedure Tfram_PutAwayContainers.btn_PrintBatchClick(Sender: TObject);
begin
  PrintList(lb_Batch, 'Putaway List');
end;

procedure Tfram_PutAwayContainers.btn_PrintErrorClick(Sender: TObject);
begin
  PrintList(lb_Error, 'Putaway Error List');
end;

procedure Tfram_PutAwayContainers.btn_ClearErrorClick(Sender: TObject);
begin
  If MessageDlg('Are you Sure',mtConfirmation,[mbYes,mbNo],0) = mrYes Then
    lb_Error.Items.Clear
end;

procedure Tfram_PutAwayContainers.btn_PrintSucessClick(Sender: TObject);
begin
  PrintList(lb_Sucess, 'Putaway Success List');
end;

procedure Tfram_PutAwayContainers.btn_ClearSucessClick(Sender: TObject);
begin
  If MessageDlg('Are you Sure',mtConfirmation,[mbYes,mbNo],0) = mrYes Then
    lb_Sucess.Items.Clear
end;

procedure Tfram_PutAwayContainers.btn_ClearBatchClick(Sender: TObject);
begin
  If MessageDlg('Are you Sure',mtConfirmation,[mbYes,mbNo],0) = mrYes Then
    lb_Batch.Items.Clear
end;

procedure Tfram_PutAwayContainers.btn_LaunchScannerClick(Sender: TObject);
var
 Filename       :String;
 I              :Cardinal;
begin
...
procedure Tfram_PutAwayContainers.btn_ProcessBatchClick(Sender: TObject);
var
  i, t                 :integer;
  mchars,mpos          :integer;
...
begin
  If (length(edt_WHouse.Text) = 0) Then
    begin
      MessageDlg('Please select a Filing Area to use for the Batch put away',mtError,[mbOK],0);
      exit;
    end;
...
//-------Process Temp Table and retrieve Results--------------------------------
    qry_SQL.Close;
    qry_SQL.SQL.Clear;
    qry_SQL.SQL.Add('EXEC [ap_ProcessBatchPutawaysContainers] :TableName, :UserId');
      ...
      begin
        If qry_SQL.FieldByName('TxResult').AsString <> 'OK' Then
          ...
          begin
            ...
            dm_Metrofiler.PrintContPutAwaySlip(qry_SQL.FieldbyName('Barcode').asString,
                                               qry_SQL.FieldbyName('Warehouse').asString,
                                               qry_SQL.FieldbyName('LocNo').asString);
          end;
        qry_SQL.Next;
      end;
...
procedure Tfram_PutAwayContainers.edt_BarcodeKeyPress(Sender: TObject;
  var Key: Char);
begin
  If ord(Key) = 13 Then
    begin
...
procedure Tfram_PutAwayContainers.edt_WHouseKeyDown(Sender: TObject;
...
procedure Tfram_PutAwayContainers.btn_GetWHouseClick(Sender: TObject);
begin
...
procedure Tfram_PutAwayContainers.btn_AddToBatchListClick(Sender: TObject);
var
  J   :Integer;
...
procedure Tfram_PutAwayContainers.ImportScanner(ImpAud :Boolean);
var
  mpos             :integer;
  mchars           :integer;
  mloc             :string;
...
procedure Tfram_PutAwayContainers.lb_BatchClick(Sender: TObject);
Var
  B, S, E     :Integer;
  Barcode     :String;
begin
...
procedure Tfram_PutAwayContainers.lb_ErrorClick(Sender: TObject);
Var
  B, S, E     :Integer;
  Barcode     :String;
begin
...
procedure Tfram_PutAwayContainers.lb_SucessClick(Sender: TObject);
Var
  B, S, E     :Integer;
  Barcode     :String;
begin
...
procedure Tfram_PutAwayContainers.PrintList(lb_Print :TListBox; Title  :String);
Var
  PageNum                 :string;
