...
type
  Tfrm_AccessFilingArea = class(TForm)
    ToolBar1: TToolBar;
    ...
    lbl_FilingAreaHd: TLabel;
    lbl_FilingArea: TLabel;
    qry_FilingArea: TADOQuery;
    Bevel1: TBevel;
  ...
  public
    { Public declarations }
    function Execute(FilingArea :String):Integer;
  end;

var
  frm_AccessFilingArea: Tfrm_AccessFilingArea;
...
procedure Tfrm_AccessFilingArea.btn_SelectClick(Sender: TObject);
var
  i : Integer;
begin
...
//---------------Delete---------------------
  ...
  qry_Access.SQL.Add('   LAuthRts');
  qry_Access.SQL.Add('WHERE');
  qry_Access.SQL.Add('   Code = :Code');
  qry_Access.Parameters.ParamValues['Code'] := mFilingArea;
  qry_Access.ExecSQL;
//---------------Add Users---------------------
        ...
        begin
          If UsersChk.Strings[I-1] = '1' Then
            begin
              qry_Access.Close;
              qry_Access.SQL.Clear;
              qry_Access.SQL.Add('INSERT INTO LAuthRts (Code, UserId) VALUES(:Code, :UserId)');
              qry_Access.Parameters.ParamByName('Code').Value := mFilingArea;
        ...
        begin
          If RolesChk.Strings[I-1] = '1' Then
            begin
              qry_Access.Close;
              qry_Access.SQL.Clear;
              qry_Access.SQL.Add('INSERT INTO LAuthRts (Code, RoleId) VALUES(:Code, :RoleId)');
              qry_Access.Parameters.ParamByName('Code').Value := mFilingArea;
...
procedure Tfrm_AccessFilingArea.DeSelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
...
procedure Tfrm_AccessFilingArea.DisplayAccess;
Var
  R  :Integer;
...
//-------------------Build Users-----------------------------------------------
  ...
  qry_Access.SQL.Add('		WHEN EXISTS(SELECT * FROM LAuthRts x WHERE x.Code = :Code AND x.UserId = a.UserId) THEN 1');
...
//-------------------Build Roles-----------------------------------------------
  ...
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN EXISTS(SELECT * FROM LAuthRts x WHERE x.Code = :Code AND x.RoleId = a.RoleId) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS Access');
  qry_Access.SQL.Add('FROM');
...
function Tfrm_AccessFilingArea.Execute(FilingArea :String):Integer;
begin
  mFilingArea := FilingArea;

  qry_FilingArea.Close;
  qry_FilingArea.SQL.Clear;
  qry_FilingArea.SQL.Add('SELECT Warehouse, Name FROM WHouse WHERE Warehouse = :Warehouse');
  qry_FilingArea.Parameters.ParamValues['Warehouse'] := mFilingArea;
  qry_FilingArea.Open;

  lbl_FilingArea.Caption := qry_FilingArea.FieldbyName('Warehouse').AsString + '-' + qry_FilingArea.FieldbyName('Name').AsString;
...
procedure Tfrm_AccessFilingArea.FormCreate(Sender: TObject);
var 
  bmp: TBitmap; 
begin 
...
procedure Tfrm_AccessFilingArea.FormDestroy(Sender: TObject);
begin
  UsersChk.Free;
  RolesChk.Free;
end;

procedure Tfrm_AccessFilingArea.MenuItem1Click(Sender: TObject);
Var
   J      :Integer;
begin
...
procedure Tfrm_AccessFilingArea.MenuItem2Click(Sender: TObject);
Var
   J      :Integer;
begin
  For J := 0 to UsersChk.Count-1 do
...
procedure Tfrm_AccessFilingArea.SelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
  For J := 0 to RolesChk.Count-1 do
...
procedure Tfrm_AccessFilingArea.sgrd_RolesClick(Sender: TObject);
var
  pt: TPoint;
  grid: TStringgrid;
  aCol, aRow: Integer;
...
procedure Tfrm_AccessFilingArea.sgrd_RolesDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_AccessFilingArea.sgrd_UsersClick(Sender: TObject);
var
  pt: TPoint;
  grid: TStringgrid;
  aCol, aRow: Integer;
...
procedure Tfrm_AccessFilingArea.ToggleCheckboxUsers( acol, arow: Integer );
begin
...
procedure Tfrm_AccessFilingArea.ToggleCheckboxRoles( acol, arow: Integer );
begin
...
procedure Tfrm_AccessFilingArea.sgrd_UsersDrawCell(Sender: TObject; ACol,
  ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  grid: TStringgrid;
begin
