...
type
  Tfrm_AccessRoleFilingArea = class(TForm)
    ToolBar1: TToolBar;
...
var
  frm_AccessRoleFilingArea: Tfrm_AccessRoleFilingArea;
...
procedure Tfrm_AccessRoleFilingArea.btn_SelectClick(Sender: TObject);
var
  i : Integer;
begin
        ...
        begin
          If AccessChk.Strings[I-1] = '1' Then
            begin
              ...
              qry_Access.Parameters.ParamByName('Code').Value := sgrd_Access.Cells[0,I];
...
procedure Tfrm_AccessRoleFilingArea.DeSelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
...
procedure Tfrm_AccessRoleFilingArea.DisplayAccess;
Var
  R  :Integer;
...
//-------------------Build DeliveryTypes----------------------------------------
  ...
  qry_Access.SQL.Add('   a.Warehouse,');
  qry_Access.SQL.Add('   a.Name,');
  qry_Access.SQL.Add('   CASE');
  qry_Access.SQL.Add('		WHEN EXISTS(SELECT * FROM LAuthRts x WHERE x.RoleId = :RoleId AND x.Code = a.Warehouse) THEN 1');
  qry_Access.SQL.Add('		ELSE 0');
  qry_Access.SQL.Add('   END AS Access');
  qry_Access.SQL.Add('FROM');
  qry_Access.SQL.Add('   WHouse a');
  ...
  MaxLen[0] := sgrd_Access.Canvas.TextWidth('Code')+10;
  ...
  While not qry_Access.EOF Do
    begin
      ...
      sgrd_Access.Cells[0,R] := qry_Access.FieldByName('Warehouse').AsString;
      sgrd_Access.Cells[1,R] := qry_Access.FieldByName('Name').AsString;
...
function Tfrm_AccessRoleFilingArea.Execute(RoleId :Integer):Integer;
Var
  TempStr   :String;
begin
  mRoleId := RoleId;
...
procedure Tfrm_AccessRoleFilingArea.FormCreate(Sender: TObject);
var 
  bmp: TBitmap; 
begin 
...
procedure Tfrm_AccessRoleFilingArea.FormDestroy(Sender: TObject);
begin
  AccessChk.free;
end;

procedure Tfrm_AccessRoleFilingArea.SelectAll1Click(Sender: TObject);
Var
   J      :Integer;
begin
  For J := 0 to AccessChk.Count-1 do
...
procedure Tfrm_AccessRoleFilingArea.sgrd_AccessClick(Sender: TObject);
var
  pt: TPoint;
  grid: TStringgrid;
  aCol, aRow: Integer;
...
procedure Tfrm_AccessRoleFilingArea.sgrd_AccessDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_AccessRoleFilingArea.ToggleCheckbox( acol, arow: Integer );
begin
