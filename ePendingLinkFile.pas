...
type
  Tfrm_ePendingLinkFile = class(TForm)
    Panel1: TPanel;
    btn_OK: TBitBtn;
    BitBtn2: TBitBtn;
    Label1: TLabel;
...
var
  frm_ePendingLinkFile: Tfrm_ePendingLinkFile;
...
function Tfrm_ePendingLinkFile.Execute:Integer;
begin
  Result := ShowModal;
end;


procedure Tfrm_ePendingLinkFile.btn_RBranchCapClick(Sender: TObject);
begin
...
procedure Tfrm_ePendingLinkFile.btn_RDeptCapClick(Sender: TObject);
begin
...
procedure Tfrm_ePendingLinkFile.btn_RRTypeCapClick(Sender: TObject);
begin
...
procedure Tfrm_ePendingLinkFile.edt_RRTypeCapChange(Sender: TObject);
Var
  RecordType   :String;
  I, J         :Integer;
  MaxLblLen    :Integer;
...
procedure Tfrm_ePendingLinkFile.btn_SearchBarcodeClick(Sender: TObject);
Var
  SelectStr     :String;
  RecordType    :String;
  J             :Integer;
...
//--------------------------------------------
  qry_SQL.Close;
  qry_SQL.SQL.Clear;
  qry_SQL.SQL.Add('SELECT Filenumber, FileType FROM Files');
  qry_SQL.SQL.Add('WHERE Branch = :mBranch');
  qry_SQL.SQL.Add('AND Department = :mDepartment');
  qry_SQL.SQL.Add('AND Filenumber = :mFile');
  ...
  qry_SQL.Parameters.ParamByName('mFile').Value := edt_Filenumber.Text;
  qry_SQL.Open;
  ...
  For J := 1 to 15 Do
    begin
      ...
      If fHeader <> '' Then
        begin
          If (J >= 1)AND(J <= 6) Then
            SelectStr := SelectStr + 'TEXT'+InttoStr(J)+' AS "'+fHeader+'",'
          else if (J >= 11)AND(J <= 12) Then
            SelectStr := SelectStr + 'DATE'+InttoStr(J-10)+' AS "'+fHeader+'",'
          else if (J >= 14)AND(J <= 15) Then
            SelectStr := SelectStr + 'NUMBER'+InttoStr(J-13)+' AS "'+fHeader+'",';
        end;
    end;
  SelectStr := SelectStr + 'IssueStatus AS "Status"'+#13;
//----
  SelectStr := SelectStr + 'FROM Files'+#13;
  SelectStr := SelectStr + 'WHERE Branch = '+QuotedStr(edt_RBranchCap.Text)+#13;
  SelectStr := SelectStr + 'AND Department = '+QuotedStr(edt_RDeptCap.Text)+#13;
  SelectStr := SelectStr + 'AND Filenumber = '+QuotedStr(edt_Filenumber.Text)+#13;

  qry_Records.Close;
  qry_Records.SQL.Clear;
  qry_Records.SQL.Add(SelectStr);
  qry_Records.Open;
end;

procedure Tfrm_ePendingLinkFile.btn_SearchClick(Sender: TObject);
Var
  SelectStr  :String;
  RecordType   :String;
  I, J         :Integer;
...
//--------------------------------------------
  qry_Records.Close;
  SelectStr := 'SELECT TOP 500 Filenumber, ';
...
//----
  ...
  SelectStr := SelectStr + 'WHERE Branch = '+QuotedStr(edt_RBranchCap.Text)+#13;
  SelectStr := SelectStr + 'AND Department = '+QuotedStr(edt_RDeptCap.Text)+#13;
  SelectStr := SelectStr + 'AND FileType = '+QuotedStr(edt_RRTypeCap.Text)+#13;
            ...
            begin
              ...
              If TEdit(FindComponent('edt_Col'+InttoStr(I))).Visible Then
                begin
                  ...
                  If fDetail <> '' Then
                    begin
                      SelectStr := SelectStr + 'AND [TEXT'+InttoStr(J)+'] LIKE '+QuotedStr(fDetail)+#13;
                    end;
                end;
            end
          else if (J >= 11)AND(J <= 12) Then
            begin
              ...
              If TEdit(FindComponent('edt_Col'+InttoStr(I))).Visible Then
                begin
                  ...
                  If fDetail <> '' Then
                    begin
                      SelectStr := SelectStr + 'AND [DATE'+InttoStr(J-10)+'] = :mDate'+InttoStr(J-10)+#13;
            ...
            begin
              ...
              If TEdit(FindComponent('edt_Col'+InttoStr(I))).Visible Then
                begin
                  ...
                  If fDetail <> '' Then
                    begin
                      SelectStr := SelectStr + 'AND [NUMBER'+InttoStr(J-13)+'] = '+fDetail+#13;
                    end;
                end;
            end;
        end;
  ...
  If fDateParam1 <> 0 Then
    qry_Records.Parameters.ParamValues['mDate1'] := fDateParam1;
  If fDateParam2 <> 0 Then
    qry_Records.Parameters.ParamValues['mDate2'] := fDateParam2;
  If fDateParam3 <> 0 Then
    qry_Records.Parameters.ParamValues['mDate3'] := fDateParam3;
  qry_Records.Open;
end;

procedure Tfrm_ePendingLinkFile.btn_OKClick(Sender: TObject);
begin
  If not qry_Records.IsEmpty Then
    begin
      qry_Det.Close;
      qry_Det.SQL.Clear;
      qry_Det.SQL.Add('SELECT Filenumber, Text1, Text2, Text3, Text4, Text5, Text6, Text7, Text8, Text9, Text10, Date1, Date2, Date3, Number1, Number2 FROM Files WHERE Filenumber = :Filenumber');
      qry_Det.Parameters.ParamByName('Filenumber').Value := trim(qry_Records.Fields[0].AsString);
      qry_Det.Open;
      qry_Records.Close;
      qry_RecordType.Close;
      ModalResult := mrOK;
